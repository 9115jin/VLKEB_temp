...
class MemoryMappedLogits:
    ...
    def __getitem__(self, idx):
        ...
        # 배열을 복사하여 쓰기 가능하게 만들고, CPU에서 생성
        array_copy = mmap_array[:].copy()
    ...
    def pop(self, idx=0):
        ...
        # 배열을 복사하여 쓰기 가능하게 만들고, CPU에서 생성
        array_copy = mmap_array[:].copy()
        result = torch.from_numpy(array_copy).float()
...
class MultimodalTrainer(BaseTrainer):
    ...
    def test_sequencial_compositional_ft(self, log: bool = False, test_num=200, gap_num=0):
        ...
        # visul-data
        base_logits_store_vis = MemoryMappedLogits()
        base_image_logits_store_vis = MemoryMappedLogits()
        # textual-data
        base_logits_store_tex = MemoryMappedLogits()
    ...
    def test_sequencial_compositional(self, log: bool = False, test_num=200, gap_num=0):
        ...
        for val_step, batch in enumerate(self.val_loader):
            if val_step < test_num:
                ...
                ### DEBUGGING ###
                print("--- 모든 Dropout 레이어 상태 점검(inference - train x) ---")
                dropout_found = False
        ...
        ### DEBUGGING ###
        print("--- 모든 Dropout 레이어 상태 점검(edit - train o) ---")
        dropout_found = False
        for name, module in self.model.named_modules():
            # 모든 Dropout 타입 체크
            if isinstance(module, (torch.nn.Dropout, torch.nn.Dropout2d, torch.nn.Dropout3d)):
        ...
        if 'llava' in self.config.model_name.lower():
            ...
            result_dir = f"results/results_sequencial/minigpt4/composition/lora"
