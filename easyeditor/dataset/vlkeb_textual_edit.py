...
class CompositionalDataset(BaseDataset):
    ...
    #     # 🔹 추가: Textual Edit 데이터 처리
    #     textual_edit = {
    #         "text_input": [" ".join([b["textual_edit"]["prompt"], b["textual_edit"]["target"]]) for b in batch],
    #         "labels": [b["textual_edit"]["target"] for b in batch],
    #         "prompts_len": [len(self.tok.encode(b["textual_edit"]["prompt"], add_special_tokens=False)) for b in batch],
    #     }
    #     textual_edit["labels"] = self.tok(
    #         textual_edit["labels"], add_special_tokens=False, return_tensors="pt"
    #     )["input_ids"]
    ...
    #         'port': edit_ports,
    #         "cond": cond,
    #         "textual_edit": textual_edit # 새로 추가한 부분
    #     }
    #     return dict_to(batch, self.config.device)
